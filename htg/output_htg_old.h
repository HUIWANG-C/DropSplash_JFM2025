//http://basilisk.fr/sandbox/sander/output_htg_old.h

#include "output_pvd_old.h"
#include "utils.h"
...
void output_htg_data_v2(scalar * list, vector * vlist, FILE * fp)
{
	...
	if(pid()==0){	  
		uint32_t prepend_size = vertices * cell_size;
	...
	for (vector v in vlist) {
		...
		for(int lvl=0; lvl<=grid->maxdepth;++lvl){
			...
			foreach_level(lvl) {
				if (is_local(cell)) {
					#if dimension == 2
					float data[3] = {v.y[],v.x[],0.};
					#elif dimension == 3
					float data[3] = {v.z[],v.y[],v.x[]};
					#endif
...
void output_htg_data_v2_xml20(scalar * list, vector * vlist, FILE * fp)
{
	...
	for (vector v in vlist) {
		//~ cell_size = dimension*sizeof(float);
		cell_size = 3*sizeof(float);
		if(pid()==0){		  
			uint32_t prepend_size = vertices * cell_size;
			fwrite(&prepend_size, sizeof(uint32_t), 1, fp); 		  
